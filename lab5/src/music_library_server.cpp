  ...
  while (msg != nullptr) {
    ...
    switch (type) {
      ...
      case MessageType::REMOVE: {
        //====================================================
        // TODO: Implement "remove" functionalit
        //====================================================
        mutex.lock();
        ...
        if(success){
          ...
          api.sendMessage(RemoveResponseMessage(remove,MESSAGE_STATUS_ERROR,"Song was not able to be removed from database"));
        }
...
int main() {
  ...
  if (server.accept(client)) {
    ...
    std::thread thread(service,lib,std::move(api),i);
    i++;
    thread.detach();
  }
}
