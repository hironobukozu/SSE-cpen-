...
int main(int argc, char* argv[]) {
  ...
  std::string maze = "data/maze1.txt";
  ...
  cpen333::process::shared_memory memory_("lab4_maze_runner",sizeof(SharedData));
  SharedData *data = (SharedData*)memory_.get();

  data->magic = 123123123;

  load_maze(std::ref(maze),std::ref(data -> minfo));

  init_runners(std::ref(data -> minfo),std::ref(data -> rinfo));

  data -> quit = false;

  std::cout << data->magic << std::endl;
